...
func init() {
	android.RegisterModuleType("lemon_generator", GeneratorFactory)
...
func (g *Module) GenerateAndroidBuildActions(ctx android.ModuleContext) {
	...
	if depRoot == "" {
		...
		depRoot = lemonExpandVariables(ctx, depRoot)
	}

	// Glob dep_files property
	for _, dep_file := range g.properties.Dep_files {
		dep_file = lemonExpandVariables(ctx, dep_file)
	...
	cmd := lemonExpandVariables(ctx, String(g.properties.Cmd))
